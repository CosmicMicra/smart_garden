...
public class Crow extends Parasite {
    ...
    public Crow(String name, int damage, String imageName, ArrayList<String> affectedPlants, int roundsToKill) {
    ...
    public void affectPlant(Plant plant) {
        if (random.nextDouble() >= MISS_CHANCE) {
            ...
            logger.info("Crow damaged {} at ({},{}). {} -> {}", plant.getName(), plant.getRow(), plant.getCol(), oldHealth, newHealth);
        } else {
            logger.info("Crow missed {} at ({},{})", plant.getName(), plant.getRow(), plant.getCol());
        }
    }
}
