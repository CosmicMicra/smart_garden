...
public class Aphids extends Parasite {
    ...
    public Aphids(String name, int damage, String imageName, ArrayList<String> affectedPlants, int roundsToKill) {
    ...
    public void affectPlant(Plant plant) {
        if (random.nextDouble() >= MISS_CHANCE) {
            ...
            logger.info("Aphid damaged {} at ({},{}). {} -> {}", plant.getName(), plant.getRow(), plant.getCol(), oldHealth, newHealth);
        } else {
            logger.info("Aphid missed {} at ({},{})", plant.getName(), plant.getRow(), plant.getCol());
        }
    }
}
