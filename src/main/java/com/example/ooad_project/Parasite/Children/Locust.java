...
public class Locust extends Parasite {
    ...
    public Locust(String name, int damage, String imageName, ArrayList<String> affectedPlants, int roundsToKill) {
    ...
    public void affectPlant(Plant plant) {
        if (random.nextDouble() >= MISS_CHANCE) {
            ...
            logger.info("Locust damaged {} at ({},{}). {} -> {}", plant.getName(), plant.getRow(), plant.getCol(), oldHealth, newHealth);
        } else {
            logger.info("Locust missed {} at ({},{})", plant.getName(), plant.getRow(), plant.getCol());
        }
    }
}
