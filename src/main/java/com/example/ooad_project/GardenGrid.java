...
//    4x4 for now
    ...
    private GardenGrid() {
        ...
        EventBus.subscribe("PlantFullyGrownEvent", event -> handleHarvest((Plant) event));
    }

    private void handleHarvest(Plant plant) {
        if (plant != null) {
            int row = plant.getRow();
            int col = plant.getCol();
            FarmerShop.getInstance().harvest(plant);
...
//        System.out.println();
    ...
    /**
     * Clears all plants from the grid. Used when re-initializing the garden.
     */
